# @title Scientific Experiment: 1-Bit vs 16-Bit (Dual Class: 0 & 1)
import torch
...
BATCH_SIZE = 128
EPOCHS = 15
TARGET_DIGITS = [0, 1] # <--- CHANGED: Now training on both
LR = 1e-3
...
# ==========================================
# 3. TRAINING ENGINE (UPDATED FOR 2 DIGITS)
# ==========================================
def train_model(model_class, name):
    ...
    print(f"Training on {len(dataset)} images of digits {TARGET_DIGITS}")
    # -------------------------------
...
def rigorous_eval(model_name, generated_imgs, training_imgs):
    ...
    # Randomly sample training data to ensure we mix 0s and 1s
...
fig.suptitle("Top: 16-Bit | Bottom: 1-Bit (Training on 0 and 1)")
for i in range(5):
...
print("\n=== CONCLUSION ===")
print(f"Memorization Gap: {abs(score_mem_16 - score_mem_1):.4f}")